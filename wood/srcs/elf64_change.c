...
/*   Updated: 2025/10/09 19:38:54 by zerrino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    int i;
    unsigned long target_pvaddr = 0;
    Elf64_Phdr *target_phdr = NULL;
    Elf64_Off max_end = 0;
    for (i = 0; i < file->elf64_ehdr->e_phnum; i++)
    {
        if (file->elf64_phdr[i]->p_type == PT_LOAD && file->elf64_phdr[i]->p_flags == 0x5)
        {
        ...
        if (target_pvaddr < (file->elf64_phdr[i]->p_vaddr + file->elf64_phdr[i]->p_memsz))
            target_pvaddr = file->elf64_phdr[i]->p_vaddr + file->elf64_phdr[i]->p_memsz;
    }
    ...
    for (size_t i = 0; i <= stub_len - pattern_len; ++i) {
        if (ft_memcmp(&stub[i], pattern, pattern_len) == 0) {
            ft_memcpy(&stub[i], &file->text_offset, sizeof(uint32_t));
        }
    }
    unsigned char pattern2[] = { 0xDD, 0xDD};
    for (size_t i = 0; i <= stub_len - pattern_len; ++i) {
        if (ft_memcmp(&stub[i], pattern2, pattern_len) == 0) {
    ...
    for (size_t i = 0; i <= stub_len - pattern_len; ++i) {
        if (ft_memcmp(&stub[i], pattern3, pattern_len) == 0) {
